...
# === GABAGOOL STRATEGY v35 (SAFE MODE) ===
# Only enter when hedge is LIKELY
# - Both sides must be reasonably priced
# - Never get stuck with one side

TOTAL_BUDGET = 8              # Total $ to spend per candle
MAX_COMBINED_AVG = 0.95       # Combined must be < 95c (profit after 2% fee)
MIN_PRICE = 0.20              # Don't buy below 20c (outcome likely decided)
MAX_PRICE = 0.55              # Don't buy above 55c (leaves room for hedge)
MAX_OTHER_PRICE = 0.55        # Other side must be ≤ 55c before we buy first side
...
def calculate_buy_amount(side, price, other_price):
    """
    SAFE GABAGOOL LOGIC:
    - First buy: ONLY if other side is also cheap enough (hedge likely)
    - Second buy: ONLY if combined < 95c
    ...
    if price > MAX_PRICE:
        return False, 0, f"Price {price*100:.0f}c too high (>{MAX_PRICE*100:.0f}c)"
    
    # No budget left
    if remaining_budget < MIN_BUY:
        return False, 0, "Budget exhausted"
    
    # CASE 1: No position at all - first buy (SAFE MODE)
    if up_shares == 0 and dn_shares == 0:
        # SAFETY CHECK: Other side must also be cheap enough!
        if other_price > MAX_OTHER_PRICE:
            return False, 0, f"Other side {other_price*100:.0f}c too high (>{MAX_OTHER_PRICE*100:.0f}c)"
        
        # Check if combined would be < 95c
        potential_combined = price + other_price
        if potential_combined >= MAX_COMBINED_AVG + 0.05:  # Give 5c buffer
            return False, 0, f"Combined {potential_combined*100:.0f}c too high"
        
        if side == "UP":
            amount = min(TOTAL_BUDGET / 2, remaining_budget)
            return True, amount, f"First buy (DN={other_price*100:.0f}c OK)"
        else:
            amount = min(TOTAL_BUDGET / 2, remaining_budget)
            return True, amount, f"First buy (UP={other_price*100:.0f}c OK)"
...
async def main():
    ...
    print("=" * 60)
    print("  BOT v35 - SAFE GABAGOOL")
    print("  Only enters when hedge is LIKELY")
    print("=" * 60)
    print(f"Total budget: ${TOTAL_BUDGET}")
    print(f"Buy price range: {MIN_PRICE*100:.0f}c - {MAX_PRICE*100:.0f}c")
    print(f"Other side max: {MAX_OTHER_PRICE*100:.0f}c (to ensure hedge possible)")
    print(f"Max combined: {MAX_COMBINED_AVG*100:.0f}c (profit after 2% fee)")
            ...
            async with websockets.connect(
                ...
                async for msg in ws:
                    ...
                    if up_shares > 0 or dn_shares > 0:
                        ...
                        status = f"BTC:{btc_change:+.2f}% | UP:{up_price*100:.0f}c DN:{dn_price*100:.0f}c"
                        
                        # Check conditions
                        up_ok = MIN_PRICE <= up_price <= MAX_PRICE
                        dn_ok = MIN_PRICE <= dn_price <= MAX_PRICE
                        
                        if up_ok and dn_ok:
                            status += " ✓ READY"
                        elif up_ok:
                            status += f" (DN {dn_price*100:.0f}c too high)"
                        elif dn_ok:
                            status += f" (UP {up_price*100:.0f}c too high)"
                        else:
                            status += " (waiting for 20c-55c)"
                        
                        print(status)
                    
                    # === SAFE GABAGOOL BUYING LOGIC ===
                    # Only buy when hedge is likely!

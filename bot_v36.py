...
# === GABAGOOL STRATEGY v36 (ULTRA SAFE) ===
# Only enter when hedge is ALMOST GUARANTEED
# - Both sides must be 40c-55c
# - Combined must be ≤ 96c already
# - Buy cheaper side first

TOTAL_BUDGET = 8              # Total $ to spend per candle
FIRST_BUY = 2                 # First buy only $2 (less risk if no hedge)
MAX_COMBINED_AVG = 0.95       # Combined must be < 95c for hedge (profit after 2% fee)
MAX_COMBINED_ENTRY = 0.96     # Only ENTER if combined ≤ 96c already
MIN_PRICE = 0.40              # Both sides must be ≥ 40c (balanced)
MAX_PRICE = 0.55              # Both sides must be ≤ 55c (balanced)
MIN_BUY = 1.0                 # Minimum buy amount ($1)
FOK_RETRIES = 3               # Retry FOK errors this many times
...
def buy_with_retry(token_id, amount, price, side):
    """Execute a buy with retries for FOK errors"""
    global up_position, dn_position
    
    for attempt in range(FOK_RETRIES):
        try:
            ...
            if resp.get("success"):
                ...
                if "fully filled" in str(err) and attempt < FOK_RETRIES - 1:
                    print(f"    ⟳ FOK retry {attempt + 1}/{FOK_RETRIES}...")
                    time.sleep(0.5)
                    continue
                print(f"    ✗ {err}")
                return False
            ...
            if attempt < FOK_RETRIES - 1:
                print(f"    ⟳ Retry {attempt + 1}/{FOK_RETRIES} after error...")
                time.sleep(0.5)
                continue
            print(f"    ✗ {e}")
            return False
...
def check_entry_conditions(up_price, dn_price):
    """
    ULTRA SAFE entry conditions:
    1. Both sides must be 40c-55c
    2. Combined must be ≤ 96c
    ...
    if up_price < MIN_PRICE:
        return False, f"UP {up_price*100:.0f}c < {MIN_PRICE*100:.0f}c"
    if up_price > MAX_PRICE:
        return False, f"UP {up_price*100:.0f}c > {MAX_PRICE*100:.0f}c"
    
    # Check DN in range
    if dn_price < MIN_PRICE:
        return False, f"DN {dn_price*100:.0f}c < {MIN_PRICE*100:.0f}c"
    if dn_price > MAX_PRICE:
        return False, f"DN {dn_price*100:.0f}c > {MAX_PRICE*100:.0f}c"
    
    # Check combined
    if combined > MAX_COMBINED_ENTRY:
        return False, f"Combined {combined*100:.0f}c > {MAX_COMBINED_ENTRY*100:.0f}c"
    
    return True, f"✓ READY ({combined*100:.0f}c combined)"


def calculate_buy_amount(side, price, other_price):
    """
    ULTRA SAFE GABAGOOL LOGIC:
    - First buy: only if BOTH sides are 40-55c AND combined ≤ 96c
    - Buy cheaper side first
    - Second buy: match shares for hedge
    """
    up_shares = up_position["shares"]
    dn_shares = dn_position["shares"]
    up_cost = up_position["cost"]
    ...
    # CASE 1: No position - first buy
    if up_shares == 0 and dn_shares == 0:
        can_enter, reason = check_entry_conditions(price, other_price)
        if not can_enter:
            return False, 0, reason
        
        # Buy the CHEAPER side first
        if side == "UP" and price <= other_price:
            return True, min(FIRST_BUY, remaining_budget), f"First buy ${FIRST_BUY} (cheaper side)"
        elif side == "DN" and price < other_price:
            return True, min(FIRST_BUY, remaining_budget), f"First buy ${FIRST_BUY} (cheaper side)"
        elif side == "UP" and price > other_price:
            return False, 0, f"DN {other_price*100:.0f}c is cheaper, buy that first"
        elif side == "DN" and price >= other_price:
            return False, 0, f"UP {other_price*100:.0f}c is cheaper, buy that first"
    
    # CASE 2: Have one side - need to hedge
    if side == "UP" and dn_shares > 0 and up_shares == 0:
        combined = price + dn_avg
        if combined >= MAX_COMBINED_AVG:
            return False, 0, f"Combined {combined*100:.0f}c ≥ {MAX_COMBINED_AVG*100:.0f}c"
...
def settle_positions(btc_final_change):
    ...
    global session_pnl, candles_traded, candles_skipped, up_position, dn_position
    ...
    if up_shares == 0 and dn_shares == 0:
        candles_skipped += 1
        print(f"\n⏭️  Candle skipped (no good entry). Total skipped: {candles_skipped}")
        return
    ...
    hedged = up_shares > 0 and dn_shares > 0
    if hedged:
        print(f"🔒 HEDGED TRADE")
    else:
        print(f"⚠️  UNHEDGED TRADE")
    ...
    session_pnl += pnl
    candles_traded += 1
    print(f"\nSession: {candles_traded} traded, {candles_skipped} skipped | Total P/L: ${session_pnl:+.2f}")
    print(f"{'='*50}\n")
...
async def main():
    ...
    print("=" * 60)
    print("  BOT v36 - ULTRA SAFE GABAGOOL")
    print("  Only enters when hedge is ALMOST GUARANTEED")
    print("=" * 60)
    print(f"Total budget: ${TOTAL_BUDGET}")
    print(f"First buy: ${FIRST_BUY} (small test)")
    print(f"Both sides must be: {MIN_PRICE*100:.0f}c - {MAX_PRICE*100:.0f}c")
    print(f"Combined must be: ≤ {MAX_COMBINED_ENTRY*100:.0f}c to enter")
    print(f"Max combined for hedge: {MAX_COMBINED_AVG*100:.0f}c")
    print(f"FOK retries: {FOK_RETRIES}")
    print("=" * 60)
            ...
            async with websockets.connect(
                ...
                async for msg in ws:
                    ...
                    if up_shares > 0 or dn_shares > 0:
                        ...
                        hedge_status = " 🔒" if (up_shares > 0 and dn_shares > 0) else ""
                        print(f"HOLD{hedge_status} | {' | '.join(pnl_parts)} | BTC:{btc_change:+.2f}%")
                    else:
                        can_enter, reason = check_entry_conditions(up_price, dn_price)
                        print(f"BTC:{btc_change:+.2f}% | UP:{up_price*100:.0f}c DN:{dn_price*100:.0f}c (={combined*100:.0f}c) | {reason}")
                    
                    # Buy logic
                    bought_this_tick = False
                    
                    should_up, up_amount, up_reason = calculate_buy_amount("UP", up_price, dn_price)
                    if should_up and not bought_this_tick:
                        print(f"\n*** UP @ {up_price*100:.0f}c - {up_reason} ***")
                        if buy_with_retry(m["up_t"], up_amount, up_price, "UP"):
                            bought_this_tick = True
                    
                    should_dn, dn_amount, dn_reason = calculate_buy_amount("DN", dn_price, up_price)
                    if should_dn and not bought_this_tick:
                        print(f"\n*** DN @ {dn_price*100:.0f}c - {dn_reason} ***")
                        if buy_with_retry(m["dn_t"], dn_amount, dn_price, "DN"):
                            bought_this_tick = True

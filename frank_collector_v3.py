        ...
        try:
            ...
            if c and c.get("up_token"):
                ...
                if fu and fd and 0.01 < fu < 0.99 and 0.01 < fd < 0.99:
                    ...
                    # Also update the scan loop's init prices
                    candle_state["up_ask_rest"] = fu
                    candle_state["dn_ask_rest"] = fd
                else:
                    print(f"  ⚠️  REST bad prices: UP={fu} DN={fd}", flush=True)
        except Exception as e:
            print(f"  ⚠️  REST poll error: {e}", flush=True)
...
async def signal_loop(shutdown_event):
    ...
    while not shutdown_event.is_set():
        ...
        # Always wait for next candle — never join mid-candle
        if elapsed_in_slot > 2:
            ...
            # Prefetch klines + enrichments in parallel during wait (NOT candle info — wrong candle)
            ...
            # Sleep remaining time to exact candle boundary

...
async def rest_price_poller(shutdown_event):
    """
    Polls CLOB REST every 5s as permanent backup.
    WebSocket is bonus — this is the reliable baseline.
    Prices always fresh, WS drops become invisible.
    """
    while not shutdown_event.is_set():
        try:
            ...
            if c and c.get("up_token"):
                ...
                if fu and fd and 0.90 <= (fu+fd) <= 1.10:
...
def get_enrichments():
    ...
    try:
        r = requests.get(f"{BINANCE_REST}/klines",
                        params={"symbol":"BTCUSDT","interval":"1m","limit":25}, timeout=4)
        klines = r.json()
        if klines:
    ...
    try:
        r = requests.get(f"{BINANCE_REST}/klines",
                        params={"symbol":"BTCUSDT","interval":"15m","limit":8}, timeout=4)
        k = r.json()
        if k and len(k)>=4:
    ...
    # OB imbalance
    try:
        r = requests.get(f"{BINANCE_REST}/depth", params={"symbol":"BTCUSDT","limit":10}, timeout=4)
        ob = r.json()
    ...
    try:
        ...
        result["funding_rate"] = round(float(r.json().get("lastFundingRate",0))*100, 6)
    except: pass
    # Agg price
    prices = {}
    for name, url, key in [
        ("binance", f"{BINANCE_REST}/ticker/price?symbol=BTCUSDT", "price"),
        ...
        try:
            ...
            d = r.json()
            if name=="coinbase": prices[name]=float(d["data"]["amount"])
            elif key: prices[name]=float(d[key])
        except: pass
    if prices:
        result["agg_btc_price"] = round(sum(prices.values())/len(prices), 2)
...
FIELDNAMES = [
    ...
    "btc_vol_ratio","macro_trend_15m","ob_imbalance","funding_rate",
    "agg_btc_price","price_sources",
]
...
async def signal_loop(shutdown_event):
    ...
    while not shutdown_event.is_set():
        ...
        scan_log.clear()
        now = time.time()
        current_slot = int(now) - (int(now) % CANDLE_DUR)
        elapsed_in_slot = now - current_slot

        if elapsed_in_slot > CANDLE_DUR * 0.50:
            wait_for = CANDLE_DUR - elapsed_in_slot
            next_slot = current_slot + CANDLE_DUR
        ...
        print("  📡 Fetching enrichments...")
        enrichments = await asyncio.get_event_loop().run_in_executor(None, get_enrichments)
        ...
        agg = enrichments.get("agg_btc_price")
        print(f"  📊 Macro:{macro} Vol:{vol_r:.1f}x OB:{ob:.2f} Fund:{fund:+.4f}%")
        if agg: print(f"  🌐 Agg: ${agg:,.2f}")

        # Clear stale prices, seed with fresh REST
        pm_buffer.clear()
        ...
        while True:
            ...
            if up_ws and dn_ws and 0.90<=(up_ws+dn_ws)<=1.10:
            ...
            # ── LIVE TRADE — try at T=120, T=150, T=180, T=210 ──
            ...
            if (in_window and not live_trade_placed
                    and candle_info and candle_info.get("slug")
                    and candle_info["slug"] not in traded_slugs
                    and utc_hour not in SKIP_HOURS
                    and clob_client is not None):

                slug = candle_info["slug"]
                ...
                if score >= MIN_SCORE and trade_dir and trade_price:
                    token = candle_info["up_token"] if trade_dir=="UP" else candle_info["dn_token"]
                    ...
                    print(f"     Win: +${payout:.2f} | Loss: -${BET:.2f}")
                    print(f"     {' | '.join(reasons[:4])}")
                    print(f"     Placing order...")

                    ok, oid, shares = place_order(token, trade_price)
                    live_trade_placed = True
                    traded_slugs.add(slug)

                    if ok:
                        print(f"  ✅ PLACED! {shares:.1f} shares | {oid[:24]}...")
                        log = load_log()
                        log["stats"]["total"]+=1; log["stats"]["pending"]+=1
                        log["trades"].append({
                            "ts": datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M"),
                            "slug": slug, "strat": strat, "dir": trade_dir,
                            "price": round(trade_price,3), "score": score, "sess": session,
                            "ok": True, "id": oid, "result": "PENDING", "pnl": None,
                            "placed_at": datetime.now(timezone.utc).isoformat(),
                            "reasons": reasons,
                            "max_up": max_up_so_far, "max_dn": max_dn_so_far,
                    ...
                    # Print skip info ONCE at final window

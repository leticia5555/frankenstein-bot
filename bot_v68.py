...
# ╔══════════════════════════════════════════════════════════════════════════╗
# ║  v68 - FRANKENSTEIN (PHYSICS ENGINE)                                    ║
# ║                                                                          ║
# ║  NEW in v68: Pure physics-based trading                                  ║
# ║  • Tracks BTC velocity (price change per second)                         ║
# ║  • Tracks acceleration (is the move speeding up?)                        ║
# ║  • Trades at minute 2 when BTC moved > 0.05% (75.5% win rate)           ║
# ║  • Fast moves at minute 2 = 78.8% win rate                              ║
# ║  • No ML needed — momentum carries through the candle                   ║
# ║  • Polymarket API outcome tracking (fixed)                               ║
# ║                                                                          ║
# ║  Based on analysis of 96 real Binance 15-min candles:                    ║
# ║  • Minute 2 big move > 0.05% predicts outcome 75.5%                     ║
# ║  • Fast velocity at min 2 = 78.8%                                       ║
# ║  • Decelerating moves = 88.9% (momentum carries)                        ║
# ║  • Reversals = smart filter (block small, allow big moves)              ║
# ╚══════════════════════════════════════════════════════════════════════════╝
...
BET_AMOUNT = 5.0
MIN_TIME_LEFT = 5.0  # v68: need 5 min left minimum

# === v68 PHYSICS PARAMETERS ===
MAX_ENTRY_PRICE = 0.58       # Max entry price
MIN_MOVE_PCT = 0.05          # Min BTC move to trigger trade (0.05%)
MIN_VELOCITY = 0.02          # Min velocity (%/min) — fast moves only
MIN_CONFIDENCE = 0.70        # v68: Only trade 70%+ confidence (was 65%)
TRADE_WINDOW_START = 1.5     # Start looking at minute 1.5
TRADE_WINDOW_END = 8.0       # Stop looking at minute 8
MAX_TRADES_PER_CANDLE = 1    # One trade per candle
...
class PhysicsEngine:
    """
    v68: Pure physics-based signal detection.
    Tracks BTC price ticks to calculate velocity, acceleration, and momentum.
    No ML — just Newton's laws applied to price movement.
    """
    def __init__(self):
        self.ticks = deque(maxlen=500)  # (timestamp, price) pairs
        self.candle_open_price = None
        self.candle_open_time = None
        self.prices_at_minutes = {}  # {minute: price}
        
    def reset(self, open_price, candle_start_timestamp=None):
        self.ticks.clear()
        self.candle_open_price = open_price
        if candle_start_timestamp:
            self.candle_open_time = candle_start_timestamp
        else:
            self.candle_open_time = time.time()
    ...
    def update(self, price, candle_open=None):
        ...
        # Keep candle open in sync with main loop
        if candle_open is not None:
            self.candle_open_price = candle_open
        
        # Record price at each minute mark
    ...
    def get_change_pct(self):
        """Current BTC change from candle open"""
    ...
    def get_velocity(self, window_seconds=30):
        """Price change per minute over last N seconds"""
        if len(self.ticks) < 5:
            return 0
        now = self.ticks[-1][0]
    ...
    def get_direction(self):
        """Current direction: 'UP', 'DN', or 'FLAT'"""
    ...
    def is_reversing(self):
        """Check if price reversed direction between minute 1 and now"""
        if 0 not in self.prices_at_minutes or not self.candle_open_price:
            return False
        
        # Direction in first minute
        ...
        # Reversal = early went one way, now going the other
    ...
    def get_signal(self, minutes_elapsed):
        """
        The v68 physics decision engine.
        Returns: (should_trade, side, confidence, reason)
        """
        change = self.get_change_pct()
        ...
        if direction == 'FLAT':
            return False, None, 0, "flat"
        
        if abs_change < MIN_MOVE_PCT:
            return False, None, 0, f"move too small ({abs_change:.3f}%)"
        
        if minutes_elapsed < TRADE_WINDOW_START:
            return False, None, 0, f"too early (min {minutes_elapsed:.1f})"
        
        if minutes_elapsed > TRADE_WINDOW_END:
            return False, None, 0, f"too late (min {minutes_elapsed:.1f})"
        
        # v68: Smart reversal handling
        # Big moves override reversal concern — momentum wins
        if reversing:
            if abs_change < 0.10:
                # Small move + reversing = skip (original 44% finding)
                return False, None, 0, f"reversing + small move ({abs_change:.3f}%)"
            else:
                # Big move + reversing = trade with penalty
                # The current direction has overwhelmed the early reversal
                pass  # Continue to confidence calc with penalty
        
        # Calculate confidence based on data findings
        confidence = 0.65  # Base: 65% for any move > 0.05%
        
        # Reversal penalty (only for moves that passed the big-move check)
        if reversing:
            confidence -= 0.05  # -5% for reversal history
        
        # Bigger moves = higher confidence
        if abs_change > 0.15:
            confidence += 0.10  # Big moves: +10%
        elif abs_change > 0.07:
            confidence += 0.05  # Medium moves: +5%
        
        # Fast velocity = higher confidence
        if abs_velocity > MIN_VELOCITY * 2:
            confidence += 0.05  # Very fast: +5%
        elif abs_velocity > MIN_VELOCITY:
            confidence += 0.02  # Fast: +2%
        
        # Minute 2-3 is optimal
        if 1.5 <= minutes_elapsed <= 3.5:
            confidence += 0.03  # Optimal window: +3%
        
        side = direction  # Trade in direction of the move
        reason = f"v={velocity:+.3f}%/m Δ={change:+.3f}%"
        
        return True, side, confidence, reason
    ...
    def get_display(self, minutes_elapsed):
        ...
        if reversing and abs_change >= 0.10:
            rev_str = " REV✓"  # Reversal but big enough to trade
        return f"Δ:{change:+.3f}% v:{velocity:+.3f}%/m {direction}{rev_str}"
...
class MLModel:
    ...
    def load_and_train(self):
        try:
            # v65: Use XGBoost instead of Random Forest
            try:
                from xgboost import XGBClassifier
                use_xgb = True
                print("[ML] Using XGBoost 🚀")
            except (ImportError, OSError, Exception) as xgb_err:
                from sklearn.ensemble import RandomForestClassifier
                use_xgb = False
                print(f"[ML] XGBoost not available, using Random Forest")
                print(f"[ML] (To enable XGBoost: brew install libomp)")
            ...
            for data_file in [DATA_FILE, "btc_15m_data_v64.csv", "btc_15m_data_v63.csv", "btc_15m_data_v62.csv", "btc_15m_data_v61.csv"]:
                try:
...
class TradeTracker:
    def __init__(self, filename="trades_v68.csv"):
...
async def run_trade_mode():
    ...
    # v68: No ML model needed — physics only
    ...
    print("=" * 65)
    print("  v68 FRANKENSTEIN - PHYSICS ENGINE")
    print(f"  Strategy: Trade momentum at minute 2-3")
    print(f"  Min BTC move: {MIN_MOVE_PCT:.2f}%")
    print(f"  Max entry price: {MAX_ENTRY_PRICE*100:.0f}¢")
    print(f"  Trade window: minute {TRADE_WINDOW_START}-{TRADE_WINDOW_END}")
    print(f"  Expected win rate: ~75% (from 96-candle analysis)")
    print("=" * 65)
                ...
                while True:
                    ...
                    if is_new:
                        if position and candle_open_btc:
                            # v65 FIX: Query Polymarket for actual outcome instead of guessing
                            prev_candle = position.get('candle_start', '')
                            actual_outcome = None
                            
                            # Try to get real outcome from Polymarket API (with retries)
                            # Wait longer — market takes time to resolve after candle ends
                            for attempt in range(8):
                                try:
                                    wait_time = [5, 8, 10, 12, 15, 20, 25, 30][attempt]
                                    if attempt == 0:
                                        log(f"[OUTCOME] Waiting {wait_time}s for market to resolve...")
                                    await asyncio.sleep(wait_time)
                                    
                                    slug = f"btc-updown-15m-{prev_candle}"
                                    r = requests.get(
                                        f"https://gamma-api.polymarket.com/events?slug={slug}",
                                        timeout=10
                                    )
                                                ...
                                                try:
                                                    ...
                                                    if prices and len(prices) >= 2:
                                                        ...
                                                        if up_final > 0.9:
                                                            ...
                                                            actual_outcome = "DN"
                                                        log(f"[OUTCOME] API: UP={up_final:.2f} DN={dn_final:.2f} → {actual_outcome}")
                                                except Exception as e:
                                                    log(f"[OUTCOME] Parse error: {e}")
                                            else:
                                                log(f"[OUTCOME] Attempt {attempt+1}: market not closed yet, retrying...")
                                    else:
                                        log(f"[OUTCOME] Attempt {attempt+1}: no data for slug {slug}")
                                    
                                    if actual_outcome:
                                        break
                                except Exception as e:
                                    log(f"[OUTCOME] Attempt {attempt+1} error: {e}")
                            
                            # Fall back to BTC price if API didn't work
                            if actual_outcome is None:
                                actual_outcome = 'UP' if btc_now > candle_open_btc else 'DN'
                                log(f"[NOTE] Used BTC price for outcome (API unavailable)")
                    ...
                    # v68: Get physics signal
                    should_trade, phy_side, phy_conf, phy_reason = physics.get_signal(minutes_elapsed)
                    ...
                    if position:
                        ...
                        entry_p = up_price if phy_side == 'UP' else dn_price
                        conf_ok = "✓" if phy_conf >= MIN_CONFIDENCE else "✗LOW"
                        print(
                            f"🎯 {phy_side} {phy_conf*100:.0f}%{conf_ok} | {phy_display} | "
                            f"entry:{entry_p*100:.0f}¢ | {minutes_left:.1f}m",
                            end='\r'
                        )
                    else:
                        print(
                    ...
                    # === v68 TRADING DECISION (PHYSICS) ===
                    if should_trade and position is None and trades_this_candle < MAX_TRADES_PER_CANDLE:
                        # Check entry price
                        if phy_side == 'UP':
                            entry_price = up_price + 0.01
                        else:
                        ...
                        if entry_price > MAX_ENTRY_PRICE:
                            ...
                            pass  # Not enough time
                        elif phy_conf < MIN_CONFIDENCE:
                            pass  # v68: Confidence too low, skip
                        else:
                            # TRADE!
                            trades_this_candle += 1
                            token_id = tokens['up'] if phy_side == 'UP' else tokens['dn']
                            
                            log(f"🎯 TRADE: {phy_side} | Physics:{phy_conf*100:.0f}% | {phy_reason}")
                            shares = place_order(client, token_id, entry_price, BET_AMOUNT, phy_side)
                            ...
                            if shares > 0:
                                ...
                                tracker.open_trade(
                                    market['timestamp'], phy_side, entry_price, shares, BET_AMOUNT,
                                    phy_conf, '-', False,
                                    0, 0, 0,
...
async def run_book_mode():
    ...
    print("=" * 65)
    print("  v65 FRANKENSTEIN - ORDER BOOK VIEWER")
...
def main():
    ...
    print("║         v68 - FRANKENSTEIN (PHYSICS ENGINE)                     ║")
    print("╠══════════════════════════════════════════════════════════════════╣")
    print("║  No ML. No filters. Just physics.                               ║")
    print("║  • Tracks BTC velocity + acceleration from Binance              ║")
    print("║  • Trades at minute 2 when move > 0.05% (75% win rate)         ║")
    print("║  • Fast moves = 78.8% win rate                                  ║")
    print("║  • Skips reversals (only 44% — worse than coin flip)            ║")
    print("║  • Polymarket API outcome tracking                              ║")
    print("╠══════════════════════════════════════════════════════════════════╣")
    print("║  MODES:                                                          ║")
    print("║  python3 bot_v68.py --trade    # Physics-based trading          ║")
    print("║  python3 bot_v68.py --analyze  # Analyze trade history          ║")
    print("╚══════════════════════════════════════════════════════════════════╝")
    print()
    ...
    if len(sys.argv) < 2:
        print("Usage:")
        print("  python3 bot_v68.py --trade    # Physics-based trading")
        print("  python3 bot_v68.py --analyze  # Analyze past trades")
        return

...
# ╔══════════════════════════════════════════════════════════════════════════╗
# ║  v71 - FRANKENSTEIN MAKER (BUDGET-CONTROLLED)                          ║
# ║                                                                          ║
# ║  EVOLUTION: v70 maker → v71 budget-controlled maker                     ║
# ║                                                                          ║
# ║  KEY CHANGES FROM v70:                                                   ║
# ║  • HEDGE ONCE: Post hedge bid once per candle, never repost             ║
# ║  • BUDGET TRACKING: Track filled amounts per side, stop when exceeded   ║
# ║  • SMART CANCEL: Only cancel MAIN orders, leave hedge alone            ║
# ║  • MAX_ENTRY_PRICE raised to 0.75 for better fill rate                 ║
# ║  • Prevents the "phantom fill" problem where cancelled orders          ║
# ║    were actually getting filled before cancel went through              ║
# ║                                                                          ║
# ║  v70 PROBLEM: Bot cancelled+reposted hedge every 30 sec.               ║
# ║  Each "cancelled" hedge was actually FILLING first → $15-25 on         ║
# ║  wrong side per candle. v71 posts hedge ONCE = max $2.50 risk.         ║
# ║                                                                          ║
...
# === v71 MAKER PARAMETERS ===
MAX_ENTRY_PRICE = 0.75         # Max price for main directional bid (raised from 0.58)
MIN_MOVE_PCT = 0.05            # Min BTC move to trigger direction call
MIN_VELOCITY = 0.02            # Min velocity (%/min)
MIN_CONFIDENCE = 0.65          # Lowered vs v68 — maker has better risk/reward
TRADE_WINDOW_START = 1.5       # Start posting at minute 1.5
TRADE_WINDOW_END = 14.0        # Keep posting until near expiry
MAX_ORDERS_PER_CANDLE = 8      # Raised — we track budget now, not count
...
ORDER_REFRESH_SECONDS = 30     # Refresh orders every 30 seconds

# === v71 BUDGET CONTROL (NEW — prevents hedge overflow) ===
MAX_MAIN_BUDGET = 15.0         # Max total spent on main directional side per candle
MAX_HEDGE_BUDGET = 2.50        # Max total spent on hedge side per candle (ONE order)
HEDGE_ONCE = True              # Post hedge only once, never repost
...
async def run_trade_mode():
    ...
    print("=" * 70)
    print("  v71 FRANKENSTEIN — BUDGET-CONTROLLED MAKER")
    print(f"  Strategy: Limit bids below market (0% fees, 0 slippage)")
    print(f"  Asset: {PRIMARY_ASSET.upper()} 15-min | Offset: {MAKER_OFFSET*100:.0f}¢ below market")
    print(f"  Hedge: ONCE only, max ${MAX_HEDGE_BUDGET:.2f} on losing side")
    print(f"  Main budget: max ${MAX_MAIN_BUDGET:.2f} per candle on winner side")
    print(f"  Expiry bids: ${EXPIRY_DEEP_BID*100:.0f}¢ both sides (last 90 sec)")
        ...
        try:
            ...
            async with websockets.connect(
                ...
                while True:
                    ...
                    # === DISPLAY ===
                    om = order_mgr.get_summary()
                    filled_sides = order_mgr.get_filled_by_side()
                    budget_info = f"M${filled_sides[phy_side if should_trade else 'UP']['cost']:.0f}/{MAX_MAIN_BUDGET:.0f} H${filled_sides[('DN' if (phy_side if should_trade else 'UP')=='UP' else 'UP')]['cost']:.0f}/{MAX_HEDGE_BUDGET:.0f}" if should_trade else ""
                    if position:
                    ...
                    # === v71 MAKER TRADING LOGIC (budget-controlled) ===
                    time_since_last_order = now - last_order_time
                    
                    if should_trade and time_since_last_order > ORDER_REFRESH_SECONDS and \
                       orders_posted_this_candle < MAX_ORDERS_PER_CANDLE:
                        
                        if phy_conf < MIN_CONFIDENCE and phase != 'expiry':
                            pass  # Confidence too low
                        elif minutes_left < 0.5:
                            pass  # Too close to expiry for new orders
                        else:
                            # === v71: CHECK BUDGETS BEFORE POSTING ===
                            # First, check what's already filled (the CLOB may have matched our orders)
                            order_mgr.check_fills(client)
                            filled = order_mgr.get_filled_by_side()
                            
                            main_side = phy_side
                            hedge_side_str = 'DN' if phy_side == 'UP' else 'UP'
                            
                            main_budget_left = order_mgr.budget_remaining(main_side, MAX_MAIN_BUDGET)
                            hedge_budget_left = order_mgr.budget_remaining(hedge_side_str, MAX_HEDGE_BUDGET)
                            
                            # DETERMINE BID PRICES
                            if phase == 'expiry':
                                main_price = max(EXPIRY_DEEP_BID,
                                    (up_price if phy_side == 'UP' else dn_price) - 0.10)
                                hedge_price = EXPIRY_DEEP_BID
                                main_size = min(BET_AMOUNT, main_budget_left)
                                hedge_size = min(BET_AMOUNT, hedge_budget_left)
                                
                                if main_size > 0.50:
                                    log(f"⚡ EXPIRY MODE: {phy_side} {phy_conf*100:.0f}% | {phy_reason}")
                                ...
                                main_size = min(BET_AMOUNT, main_budget_left)
                                hedge_size = min(BET_AMOUNT * HEDGE_RATIO, hedge_budget_left)
                                
                                if main_size > 0.50:
                                    log(f"📝 MAKER BID: {phy_side} {phy_conf*100:.0f}% | {phy_reason}")
                            
                            else:  # early
                                main_price = 0.48
                                hedge_price = 0.48
                                main_size = min(BET_AMOUNT * 0.5, main_budget_left)
                                hedge_size = min(BET_AMOUNT * 0.5, hedge_budget_left)
                                
                                if main_size > 0.50:
                                    log(f"📝 EARLY BID: {phy_side} {phy_conf*100:.0f}% | {phy_reason}")
                            
                            # === v71: Only cancel MAIN orders, not hedge ===
                            # Cancel only the main side orders (leave hedge alone if posted)
                            for order_id, order in list(order_mgr.active_orders.items()):
                                if 'MAIN_' in order.get('label', ''):
                                    order_mgr.cancel_order(client, order_id)
                            
                            # POST MAIN BID (only if we have budget)
                            if main_size >= 0.50:
                                ...
                                if main_order:
                                    ...
                                    if main_order.get('status') == 'filled' and position is None:
                                        ...
                                        log(f"  🎯 INSTANT POSITION: {phy_side} {filled_shares:.1f} shares @ {main_price*100:.0f}¢")
                            else:
                                if main_budget_left < 0.50:
                                    pass  # Budget exhausted — silent
                            
                            # POST HEDGE BID — v71: ONLY ONCE per candle
                            if not direction_locked and not hedge_posted and hedge_size >= 0.50:
                                ...
                                if hedge_order:
                                    hedge_posted = True  # v71: Never post another hedge
                            ...
                            # Check if we should lock direction (cancel hedge)
                            if phy_conf >= DIRECTION_LOCK_CONF and not direction_locked:
                                cancelled = order_mgr.cancel_side(client, f"HEDGE_{hedge_side_str}")
                                if cancelled > 0:
                                    direction_locked = True
                                    log(f"🔒 Direction locked: {phy_side} ({phy_conf*100:.0f}%) — hedge cancelled")
...
def main():
    print()
    print("╔══════════════════════════════════════════════════════════════════════╗")
    print("║         v71 - FRANKENSTEIN MAKER (BUDGET-CONTROLLED)                ║")
    print("╠══════════════════════════════════════════════════════════════════════╣")
    print("║  The maker strategy: post limit bids, earn rebates, zero fees.       ║")
    print("║  • Physics engine predicts direction (from v68)                      ║")
    print("║  • Posts maker bids below market (0% fee, 0 slippage)               ║")
    print("║  • HEDGE ONCE: max $2.50 on losing side (v70 fix)                  ║")
    print("║  • Budget tracking: prevents phantom fill overflow                   ║")
    print("║  • Cancels hedge when direction locks (>80% confidence)             ║")
    print("║  • Expiry mode: deep bids in last 90 seconds                        ║")
    print("║  • Multi-asset: SOL (best) > ETH > BTC                             ║")
    print("║  • Validated from whale k9Q2mX4L8A7ZP3R trades ($627K profit)       ║")
    print("╠══════════════════════════════════════════════════════════════════════╣")
    print("║  MODES:                                                              ║")
    print("║  python3 bot_v71.py --trade    # Maker trading (SOL 15-min)         ║")
    print("║  python3 bot_v71.py --scan     # Multi-asset scanner               ║")
    print("║  python3 bot_v71.py --analyze  # Analyze trade history              ║")
    print("╚══════════════════════════════════════════════════════════════════════╝")
    print()

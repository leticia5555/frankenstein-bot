            ...
            async with websockets.connect(
                ...
                async for msg in ws:
                    ...
                    # LOOKING FOR ENTRY
                        ...
                        # === CHEAP BUY MODE ===
                        # If price is really cheap AND trend confirms, buy it
                        # Only buy UP if BTC is going up, only buy DN if BTC is going down
                        if cheap_buys_this_candle < MAX_CHEAP_BUYS:
                            if m["up"] <= CHEAP_BUY_PRICE and btc_change >= 0 and not position:
                                print(f"*** 🛒 CHEAP BUY #{cheap_buys_this_candle+1}: UP @ {m['up']*100:.0f}c (BTC {btc_change:+.2f}%) ***")
                                if buy(m["up_t"], m["up"], "UP"):
                                    cheap_buys_this_candle += 1
                                continue
                            
                            if m["dn"] <= CHEAP_BUY_PRICE and btc_change <= 0 and not position:
                                print(f"*** 🛒 CHEAP BUY #{cheap_buys_this_candle+1}: DN @ {m['dn']*100:.0f}c (BTC {btc_change:+.2f}%) ***")
                                if buy(m["dn_t"], m["dn"], "DN"):
                                    cheap_buys_this_candle += 1
                                continue

...
def check_for_new_candle():
    global last_candle_time, candle_open, recent_prices
...
def check_for_new_market():
    global position, recent_prices, position_is_sure_thing
...
async def main():
    global position, candle_open, tokens, last_candle_time, recent_prices, position_is_sure_thing
    
    print("=" * 60)
    print("  BOT v8 - MOMENTUM + SURE THING + ARBITRAGE")
    print("=" * 60)
    ...
    print(f"Sure Thing: BTC ≥{SURE_THING_BTC_MOVE}% OR price ≥80c (trust market!)")
    print(f"Arbitrage: Buy BOTH if UP+DN ≤ {ARB_MAX_COMBINED*100:.0f}c")
    print("=" * 60)
                ...
                async for msg in ws:
                    ...
                    # LOOKING FOR ENTRY
                        ...
                        if combined_price <= ARB_MAX_COMBINED:
                            ...
                            # Clear position since we're holding both (will settle at expiry)
                            position = None
                            continue
                        ...
                        if best_edge >= MIN_EDGE and best_price <= MAX_PRICE:
                            if can_trade:
                                ...
                                buy(best_token, best_price, best_side)
                            else:

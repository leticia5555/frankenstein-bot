...
# ╔══════════════════════════════════════════════════════════════════════════╗
# ║  v70 - FRANKENSTEIN MAKER                                               ║
# ║                                                                          ║
# ║  EVOLUTION: v68 physics taker → v70 maker strategy                       ║
# ║                                                                          ║
# ║  KEY CHANGES FROM v68:                                                   ║
# ║  • MAKER orders (limit bids below market) instead of taker               ║
# ║  • ZERO fees (makers pay 0%, takers pay up to 1.56%)                     ║
# ║  • ZERO slippage (we set our price)                                      ║
# ║  • PLUS rebates (100% of taker fees redistributed to makers)             ║
# ║  • Dual-side posting: strong bid on predicted winner + hedge on loser    ║
# ║  • Order cancellation when direction becomes clear                       ║
# ║  • Expiry window: aggressive bids in last 60 seconds                     ║
# ║  • Multi-asset scanning (SOL > ETH > BTC by edge quality)               ║
# ║                                                                          ║
...
MAKER_OFFSET = 0.02            # Bid this far below market (3 cents)
HEDGE_PRICE = 0.40             # Hedge bid price on losing side
HEDGE_RATIO = 0.50             # Hedge size = 30% of main bet
FEE_RATE_BPS = 1000            # 15-min crypto markets fee rate

# === v70 MAKER PARAMETERS ===
...
MAX_ORDERS_PER_CANDLE = 6      # Can post multiple limit orders
...
async def run_trade_mode():
    ...
    print("=" * 70)
    print("  v70 FRANKENSTEIN — MAKER ENGINE")
    ...
    print(f"  Hedge: {HEDGE_RATIO*100:.0f}% @ {HEDGE_PRICE*100:.0f}¢ on losing side")
                       ...
                       orders_posted_this_candle < MAX_ORDERS_PER_CANDLE:
                        ...
                        if phy_conf < MIN_CONFIDENCE and phase != 'expiry':
                            ...
                            if phy_conf >= DIRECTION_LOCK_CONF and not direction_locked:
                                hedge_side = 'DN' if phy_side == 'UP' else 'UP'
                                cancelled = order_mgr.cancel_side(client, f"HEDGE_{hedge_side}")
                                if cancelled > 0:
...
def main():
    ...
    print("║  • Dual-side: main bid on winner + hedge on loser                   ║")
    ...
    print("╠══════════════════════════════════════════════════════════════════════╣")
    print("║  MODES:                                                              ║")
    print("║  python3 bot_v70.py --trade    # Maker trading (SOL 15-min)         ║")
    print("║  python3 bot_v70.py --scan     # Multi-asset scanner               ║")
    print("║  python3 bot_v70.py --analyze  # Analyze trade history              ║")
    print("╚══════════════════════════════════════════════════════════════════════╝")
    print()

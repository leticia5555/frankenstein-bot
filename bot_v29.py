...
MAX_PRICE = 0.53         # Don't buy if price > 53c (edge trades only - buy cheap!)
PROFIT_TARGET = 0.05     # Take profit at +5c
STOP_LOSS = 0.06         # Stop loss at -6c

# === CHEAP BUY SETTINGS ===
CHEAP_BUY_PRICE = 0.40   # Buy if price ≤ 40c (no edge needed!)
MAX_CHEAP_BUYS = 2       # Max cheap buy trades per candle
...
cheap_buys_this_candle = 0      # Count cheap buy trades (max 2 per candle)
...
def sell(reason):
    ...
    try:
        ...
        if "price" in error_str and ("0.999" in error_str or "min: 0.01" in error_str):
            ...
            position_is_sure_thing = False
            position_is_lag_trade = False
            return True  # Return True to stop retry loop
        else:
            print(f"    ✗ Error: {e}")
            return False
...
def check_for_new_candle():
    global last_candle_time, candle_open, recent_prices, has_traded_this_candle, edge_trades_up, edge_trades_dn, cheap_buys_this_candle, position_is_sure_thing, position_is_lag_trade
    ...
    if new_time != last_candle_time:
        ...
        cheap_buys_this_candle = 0      # Reset cheap buy counter
...
def check_for_new_market():
    global position, recent_prices, position_is_sure_thing, position_is_lag_trade, has_traded_this_candle, edge_trades_up, edge_trades_dn, cheap_buys_this_candle, session_pnl, trade_count
    
    is_new = find_active_market()
    if is_new:
        if position:
            # OLD MARKET EXPIRED - tokens settled automatically
            # Don't try to sell - just clear position and estimate P/L
            print(f"\n<<< MARKET EXPIRED - Position auto-settled")
            # If we held to expiry, assume we won (since Sure Thing should win)
            # This is an estimate - actual P/L depends on settlement
        ...
        cheap_buys_this_candle = 0      # Reset for new market
        return True
    return False


async def main():
    global position, candle_open, tokens, last_candle_time, recent_prices, position_is_sure_thing, position_is_lag_trade, has_traded_this_candle, edge_trades_up, edge_trades_dn, cheap_buys_this_candle, lag_prices
    
    print("=" * 60)
    print("  BOT v24 - CHEAP BUY FOLLOWS TREND")
    print("=" * 60)
    print(f"CHEAP BUY: ≤{CHEAP_BUY_PRICE*100:.0f}c + trend confirms")
    print(f"EDGE: {MIN_EDGE*100:.0f}c min | ≤{MAX_PRICE*100:.0f}c | 2 UP + 2 DN")
    print(f"LAG: {LAG_BTC_SPIKE}% spike | ≤{LAG_MAX_PRICE*100:.0f}c")
    print(f"SURE THING: BTC ≥{SURE_THING_BTC_MOVE}%")
    print(f"TP: +{PROFIT_TARGET*100:.0f}c | SL: -{STOP_LOSS*100:.0f}c")
    print("=" * 60)
            ...
            async with websockets.connect(
                ...
                async for msg in ws:
                    ...
                    if position:
                        ...
                        if position_is_sure_thing:
                            # Sure Thing = take profit at +5c, but no stop loss (BTC is on our side)
                            if price_diff >= PROFIT_TARGET:
                                sell(f"SURE PROFIT +{price_diff*100:.0f}c")
                        elif position_is_lag_trade:
                            # Lag trade = quick exit at +2c or stop at -6c
                            if price_diff >= LAG_PROFIT_TARGET:
                                sell(f"LAG PROFIT +{price_diff*100:.0f}c")
                            elif price_diff <= -0.06:  # Same stop as regular trades
                    ...
                    # LOOKING FOR ENTRY
                        ...
                        # Execute Sure Thing - DISABLED FOR NOW
                        ...
                        # === CHEAP BUY MODE ===
                        # If price is really cheap AND momentum confirms, buy it
                        # Requires actual momentum signal, not just BTC slightly positive/negative
                        if cheap_buys_this_candle < MAX_CHEAP_BUYS:
                            if m["up"] <= CHEAP_BUY_PRICE and momentum == "UP" and not position:
                                print(f"*** 🛒 CHEAP BUY #{cheap_buys_this_candle+1}: UP @ {m['up']*100:.0f}c (MOMENTUM 🟢UP) ***")
                                if buy(m["up_t"], m["up"], "UP"):
                                    cheap_buys_this_candle += 1
                                continue
                            
                            if m["dn"] <= CHEAP_BUY_PRICE and momentum == "DOWN" and not position:
                                print(f"*** 🛒 CHEAP BUY #{cheap_buys_this_candle+1}: DN @ {m['dn']*100:.0f}c (MOMENTUM 🔴DN) ***")
                                if buy(m["dn_t"], m["dn"], "DN"):
                                    cheap_buys_this_candle += 1
                                continue

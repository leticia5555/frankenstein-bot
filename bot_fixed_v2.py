...
TRADE_AMOUNT = 2
EDGE_THRESHOLD = 0.08      # Lowered from 0.10 (8c edge to enter)
TREND_THRESHOLD = 0.03     # Lowered from 0.05 (0.03% BTC move = trend)
PROFIT_TARGET = 0.04       # Take profit at +4c
...
async def main():
    ...
    print("=" * 60)
    print("  POLYMARKET BTC 15M TREND BOT v2")
    print("=" * 60)
    print(f"Trade Size:      ${TRADE_AMOUNT}")
    print(f"Trend Threshold: {TREND_THRESHOLD*100:.2f}% BTC move")
            ...
            async with websockets.connect(
                ...
                async for msg in ws:
                    ...
                    if btc_change > TREND_THRESHOLD:
                        ...
                        trend_token = None
                        trend_price = 0
                        # Show the better edge when flat
                    ...
                    if position:
                        ...
                        if profit >= PROFIT_TARGET:
                            ...
                            sell("STOP LOSS")
                        elif position["side"] == "UP" and btc_change < -TREND_THRESHOLD:
                            sell("TREND FLIP TO DOWN")
                        elif position["side"] == "DN" and btc_change > TREND_THRESHOLD:
                            sell("TREND FLIP TO UP")
                    
                    # If no position, look for entry
                    else:
                        # Show both edges when flat for visibility
                        if trend == "FLAT":
                            print(f"BTC:{btc_change:+.3f}% | {trend} | UP:{m['up']*100:.0f}c(+{up_edge*100:.0f}c) DN:{m['dn']*100:.0f}c(+{dn_edge*100:.0f}c)")
                        else:
                            print(f"BTC:{btc_change:+.3f}% | {trend} | UP:{m['up']*100:.0f}c DN:{m['dn']*100:.0f}c | Edge:+{edge*100:.0f}c")
                        
                        # Entry conditions
                        if trend == "UP" and edge >= EDGE_THRESHOLD and m["up"] < 0.85:
                            print(f"*** UP trend + {edge*100:.0f}c edge! BUYING ***")
                            buy(trend_token, trend_price, "UP")
                        
                        elif trend == "DN" and edge >= EDGE_THRESHOLD and m["dn"] < 0.85:
                            print(f"*** DN trend + {edge*100:.0f}c edge! BUYING ***")

...
SURE_THING_PRICE_TRIGGER = 0.85  # If price is ≥85c, market has decided - lock it in!

# === ARBITRAGE SETTINGS ===
ARB_MAX_COMBINED = 0.94       # Buy both sides if UP + DOWN ≤ 94c (6c buffer for slippage)
...
def check_for_new_candle():
    global last_candle_time, candle_open, recent_prices, has_traded_this_candle
...
def check_for_new_market():
    global position, recent_prices, position_is_sure_thing, has_traded_this_candle
...
async def main():
    global position, candle_open, tokens, last_candle_time, recent_prices, position_is_sure_thing, has_traded_this_candle
    
    print("=" * 60)
    print("  BOT v10 - ONE TRADE PER CANDLE")
    print("=" * 60)
    print(f"Min Edge: {MIN_EDGE*100:.0f}c | Max Price: {MAX_PRICE*100:.0f}c")
    print(f"TP: +{PROFIT_TARGET*100:.0f}c | SL: -{STOP_LOSS*100:.0f}c")
    print(f"Min Momentum: {MIN_MOMENTUM}% from candle open")
    print(f"Sure Thing: BTC ≥{SURE_THING_BTC_MOVE}% OR price ≥80c (HOLD TO EXPIRY)")
    print(f"Arbitrage: Buy BOTH if UP+DN ≤ {ARB_MAX_COMBINED*100:.0f}c (HOLD TO EXPIRY)")
    print("=" * 60)
            ...
            async with websockets.connect(
                ...
                async for msg in ws:
                    ...
                    # LOOKING FOR ENTRY
                        ...
                        # Based on trader strategy: if price ≥80c, market has decided!
                        ...
                        # Method 2: Price is very high (≥80c) = market has decided!
                        # Trust the MARKET price, not BTC direction
                        elif m["up"] >= 0.80 and m["up"] <= SURE_THING_MAX_PRICE:
                            sure_thing = True
                            sure_side = "UP"
                            sure_token = m["up_t"]
                            sure_price = m["up"]
                        
                        elif m["dn"] >= 0.80 and m["dn"] <= SURE_THING_MAX_PRICE:
                            sure_thing = True
                            sure_side = "DN"
                            sure_token = m["dn_t"]
                            sure_price = m["dn"]
                        
                        if sure_thing and (1 - sure_price) >= SURE_THING_MIN_PROFIT:
                            ...
                            if buy(sure_token, sure_price, sure_side):
                                position_is_sure_thing = True  # Mark as Sure Thing - HOLD TO EXPIRY!
                                has_traded_this_candle = True  # Only one trade per candle!
                            continue  # Skip normal logic, we already bought
                        ...
                        # Entry: edge good + price cheap + momentum confirms
                        if best_edge >= MIN_EDGE and best_price <= MAX_PRICE:
                            if can_trade:
                                print(f"*** {best_side} SIGNAL: +{best_edge*100:.0f}c edge @ {best_price*100:.0f}c | MOMENTUM ✓ ***")

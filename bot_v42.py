...
# === GABAGOOL STRATEGY v42 (SAFE DOUBLE DOWN) ===
# Based on v41 but with SAFER double down logic
# 1. Try to hedge if both sides 40-55c
# 2. Double down ONLY when:
#    - Share price ≥70c
#    - Time remaining < 7 minutes (less reversal risk)
#    - BTC momentum > ±0.15% (strong trend)
#    - BTC direction matches DD side
# 3. OR buy winner directly at ≥70c if momentum is clear
...
DD_MIN_MOMENTUM = 0.15        # DD requires BTC > ±0.15% (NEW)
DD_MAX_TIME_LEFT = 7.0        # DD only when < 7 minutes left (NEW)
        ...
        try:
            async with websockets.connect(
                ...
                async for msg in ws:
                    ...
                    # SAFE DD: Only when time is short AND momentum is strong
                    if is_hedged() and not doubled_down and remaining >= 1:
                        
                        # Check DD safety conditions
                        dd_time_ok = minutes_left < DD_MAX_TIME_LEFT
                        dd_momentum_strong = abs(btc_change) >= DD_MIN_MOMENTUM
                        
                        if dd_time_ok and dd_momentum_strong:
                            # DD on UP only if BTC is actually going UP
                            if (up_price >= WINNER_THRESHOLD and 
                                up_price <= MAX_WINNER_PRICE and 
                                btc_change > 0):
                                buy_amount = min(WINNER_BUY, remaining)
                                print(f"\n*** 🚀 SAFE DD: UP @ {up_price*100:.0f}c (BTC {btc_change:+.2f}%, {minutes_left:.1f}m left) ***")
                                if buy_with_retry(m["up_t"], buy_amount, up_price, "UP"):
                                    doubled_down = True
                            
                            # DD on DN only if BTC is actually going DOWN
                            elif (dn_price >= WINNER_THRESHOLD and 
                                  dn_price <= MAX_WINNER_PRICE and 
                                  btc_change < 0):
                                buy_amount = min(WINNER_BUY, remaining)
                                print(f"\n*** 🚀 SAFE DD: DN @ {dn_price*100:.0f}c (BTC {btc_change:+.2f}%, {minutes_left:.1f}m left) ***")
                                if buy_with_retry(m["dn_t"], buy_amount, dn_price, "DN"):
                                    doubled_down = True
                        
                        elif up_price >= WINNER_THRESHOLD or dn_price >= WINNER_THRESHOLD:
                            # Log why we're NOT doing DD
                            if not dd_time_ok:
                                pass  # Too much time left, staying hedged
                            elif not dd_momentum_strong:
                                pass  # Momentum too weak, staying hedged

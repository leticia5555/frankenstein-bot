...
# === GABAGOOL STRATEGY v47 - PRICE TO BEAT ===
# KEY FIXES:
# 1. Fetch REAL "price to beat" from Polymarket market description
# 2. BTC % calculated from oracle's actual open price
# 3. No more opening price mismatch!
# 4. Chainlink feed via Polymarket RTDS
...
NO_DD_AFTER_MINUTES = 3.0     # Don't DD in last 3 minutes

SLUG = None
tokens = None
price_to_beat = None  # The REAL oracle open price
candle_start_time = None

# Chainlink price from RTDS
chainlink_btc_price = None
...
def get_minutes_remaining():
    """Minutes until market settlement"""
...
def fetch_price_to_beat_from_api():
    """Fetch the REAL 'price to beat' from Polymarket market data"""
    ...
    try:
        ...
        if data and len(data) > 0:
            ...
            if markets:
                ...
                # Check question and description for price
                question = market.get("question", "")
                desc = market.get("description", "")
                full_text = question + " " + desc
                
                # Pattern: "$88,152.73" or "$88152.73"
                price_match = re.search(r'\$([0-9,]+\.[0-9]+)', full_text)
                if price_match:
                    price_str = price_match.group(1).replace(',', '')
                    price_to_beat = float(price_str)
                    print(f"✓ Price to beat from API: ${price_to_beat:,.2f}")
                    return price_to_beat
                    
    except Exception as e:
        print(f"Error fetching price to beat: {e}")
    
    return None


def find_active_market():
    global SLUG, tokens, candle_start_time, price_to_beat
                ...
                if markets:
                    ...
                    if market.get("acceptingOrders") and not market.get("closed"):
                        if expected_slug != SLUG:
                            ...
                            # Reset price_to_beat for new market
                            price_to_beat = None
                            fetch_price_to_beat_from_api()
...
async def main():
    global SLUG, tokens, price_to_beat, candle_start_time
    global doubled_down, bought_winner_direct
    global chainlink_btc_price, session_pnl
    
    print("=" * 50)
    print("GABAGOOL v47 - PRICE TO BEAT")
    print("Uses REAL oracle open price from Polymarket")
    print("=" * 50)
        ...
        try:
            ...
            async with websockets.connect("wss://ws-live-data.polymarket.com") as ws:
                ...
                while True:
                    ...
                    # Parse Chainlink price
                    if data.get("type") == "oracle" or "price" in data:
                        price_data = data.get("data", data)
                        if isinstance(price_data, dict) and "price" in price_data:
                            chainlink_btc_price = float(price_data["price"])
                        elif "price" in data:
                            chainlink_btc_price = float(data["price"])
                    
                    if not chainlink_btc_price:
                        continue
                    
                    btc_now = chainlink_btc_price
                    tick_count += 1
                    
                    # Set fallback if no price_to_beat found
                    if fallback_open is None and price_to_beat is None:
                        fallback_open = btc_now
                        print(f"Fallback open (Chainlink): ${fallback_open:,.2f}")
                    ...
                    if tick_count % 30 == 0:
                        ...
                        if find_active_market():
                            ...
                            if price_to_beat:
                                ...
                                fallback_open = btc_now
                                print(f"*** Fallback open: ${fallback_open:,.2f} ***")
                            print()
                    ...
                    # Calculate BTC % using REAL price to beat
                    reference_price = price_to_beat if price_to_beat else fallback_open
                    if reference_price and reference_price > 0:
                        btc_change = (btc_now - reference_price) / reference_price * 100
                    ...
                    # Direction indicator based on BTC %
                    direction = "UP" if btc_change > 0 else "DN" if btc_change < 0 else "FLAT"
                    ...
                    if is_hedged():
                        ...
                        if up_shares > dn_shares + 0.5:
                            ...
                            extra = " 📉+DN"
                        print(f"🔒{extra} | UP:{up_shares:.1f}(${up_pnl:+.2f}) DN:{dn_shares:.1f}(${dn_pnl:+.2f}) | BTC:{btc_change:+.2f}% ({direction}) | {minutes_left:.1f}m")
                    elif has_position():
                        if up_shares > 0:
                            pnl = (up_price - up_position["avg_price"]) * up_shares
                            print(f"🎯 UP | {up_shares:.1f}@{up_position['avg_price']*100:.0f}c(${pnl:+.2f}) | BTC:{btc_change:+.2f}% ({direction}) | {minutes_left:.1f}m")
                        else:
                            pnl = (dn_price - dn_position["avg_price"]) * dn_shares
                            print(f"🎯 DN | {dn_shares:.1f}@{dn_position['avg_price']*100:.0f}c(${pnl:+.2f}) | BTC:{btc_change:+.2f}% ({direction}) | {minutes_left:.1f}m")
                    else:
                        ...
                        print(f"👀 | UP:{up_price*100:.0f}c DN:{dn_price*100:.0f}c | BTC:{btc_change:+.2f}% ({direction}) | {minutes_left:.1f}m | {reason}{winner_opp}")
                    ...
                    # === STRATEGY 2: DOUBLE DOWN (not in last 3 min!) ===
                    if is_hedged() and not doubled_down and remaining >= 1:
                        if minutes_left > NO_DD_AFTER_MINUTES:

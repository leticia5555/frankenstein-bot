...
# ╔══════════════════════════════════════════════════════════════════════════╗
# ║  v65 - FRANKENSTEIN BOT (PICKY TRADER + XGBOOST)                        ║
# ║                                                                          ║
# ║  NEW in v65:                                                             ║
# ║  • XGBoost replaces Random Forest (better accuracy)                      ║
# ║  • RSI filter: no UP when RSI>95, no DN when RSI<5                       ║
# ║  • Max entry price 63¢ (better risk/reward)                              ║
# ║  • Require book signal OR ML>72% (no blind trades)                       ║
# ║  • No entries after minute 8 (no late gambling)                          ║
# ║  • Min EV raised to 8% (no thin-edge trades)                            ║
# ║  • Fewer trades, higher quality = more profit                            ║
# ║                                                                          ║
# ║  KEPT from v64:                                                          ║
# ║  • Crash-proof position persistence                                      ║
# ║  • Order book analysis                                                   ║
# ╚══════════════════════════════════════════════════════════════════════════╝

# === PARAMETERS ===
DATA_FILE = "btc_15m_data_v65.csv"
MIN_CANDLES_TO_TRAIN = 96
BET_AMOUNT = 5.0
MIN_TIME_LEFT = 7.0  # v65: raised from 3.0 — no entries after minute 8

# === v65 PARAMETERS (tightened from data analysis) ===
MIN_EXPECTED_VALUE = 0.08    # v65: raised from 0.03 — no thin-edge trades
CONFIDENCE_FLOOR = 0.60
ALPHA_EXTRACTION = 0.90
MAX_TRADES_PER_CANDLE = 1    # v65: reduced from 2 — one good trade per candle
MAX_ENTRY_PRICE = 0.58       # v65b: sweet spot — good payouts, enough trades
RSI_OVERBOUGHT = 95          # v65: NEW — don't buy UP above this
RSI_OVERSOLD = 5             # v65: NEW — don't buy DN below this
REQUIRE_BOOK_OR_HIGH_ML = True  # v65: NEW — need book signal OR ML>72%
HIGH_ML_THRESHOLD = 0.72     # v65: NEW — ML confidence to trade without book
...
class TradeTracker:
    def __init__(self, filename="trades_v65.csv"):
...
async def run_trade_mode():
    ...
    print("=" * 65)
    print("  v65 FRANKENSTEIN - PICKY TRADER MODE (XGBOOST + FILTERS)")
    ...
    print(f"  RSI Limits: UP<{RSI_OVERBOUGHT} DN>{RSI_OVERSOLD}")
    print(f"  Min Time Left: {MIN_TIME_LEFT:.0f}m")
    print(f"  Need: Book signal OR ML>{HIGH_ML_THRESHOLD*100:.0f}%")
    print("=" * 65)
            ...
            async with websockets.connect("wss://stream.binance.com:9443/ws/btcusdt@trade") as ws:
                ...
                while True:
                    ...
                    if is_new:
                        if position and candle_open_btc:
                            ...
                            for attempt in range(3):
                                try:
                                    if attempt > 0:
                                        await asyncio.sleep(2)  # Wait for market to resolve
                        ...
                        if market:
                            ...
                            # v65b: Force immediate book check on new candle
                            last_book_check = 0
                    ...
                    if position:
                        ...
                        book_str = f"📚{book_pred}" if book_signal and book_signal.get('has_signal') else "📚-"
                        agree = "✓" if book_signal and book_signal.get('has_signal') and book_signal.get('prediction') == prediction else ""
                        
                        # v65: Show filter status
                        filters = []
                        if prediction == 'UP' and rsi > RSI_OVERBOUGHT:
                            filters.append("RSI!")
                        if prediction == 'DN' and rsi < RSI_OVERSOLD and rsi > 0:
                            filters.append("RSI!")
                        if prediction == 'UP' and up_price > MAX_ENTRY_PRICE:
                            filters.append("$$!")
                        if prediction == 'DN' and dn_price > MAX_ENTRY_PRICE:
                            filters.append("$$!")
                        ...
                        print(
                            ...
                            f"RSI:{rsi:.0f} | {minutes_left:.1f}m{filter_str}",
                            end='\r'
                        )
                    else:
                        print(
                    ...
                    # === TRADING DECISION (v65: PICKY FILTERS) ===
                    should_trade = (
                    ...
                    # v65: RSI filter — don't fight extreme momentum
                    # Skip RSI check in first minute (RSI not yet calculated)
                    if should_trade and minutes_elapsed >= 1:
                        if prediction == 'UP' and rsi > RSI_OVERBOUGHT:
                            should_trade = False
                        elif prediction == 'DN' and rsi < RSI_OVERSOLD and rsi > 0:
                            should_trade = False
                    
                    # v65: Max entry price — no expensive entries
                    if should_trade:
                        if prediction == 'UP' and up_price > MAX_ENTRY_PRICE:
                            should_trade = False
                        elif prediction == 'DN' and dn_price > MAX_ENTRY_PRICE:
                            should_trade = False
                    
                    # v65: Require book signal OR high ML confidence
                    if should_trade and REQUIRE_BOOK_OR_HIGH_ML:
                        has_book = book_signal and book_signal.get('has_signal') and book_signal.get('prediction') == prediction
                        has_high_ml = ml_confidence >= HIGH_ML_THRESHOLD
                        if not has_book and not has_high_ml:
                            should_trade = False
                    
                    # Book-based filters (kept from v64)
                    if should_trade and book_signal:
                        ...
                        if book_signal.get('has_signal') and book_signal.get('prediction') != prediction:
                            if book_signal.get('signal_strength', 0) > 0.3:
                                should_trade = False
                    
                    # Price range filter (tightened for v65)
                    if should_trade:
                        if prediction == 'UP' and not (0.30 <= up_price <= MAX_ENTRY_PRICE):
                            should_trade = False
                        elif prediction == 'DN' and not (0.30 <= dn_price <= MAX_ENTRY_PRICE):
                            should_trade = False
...
def main():
    print()
    print("╔══════════════════════════════════════════════════════════════════╗")
    print("║         v65 - FRANKENSTEIN (PICKY TRADER + XGBOOST)            ║")
    print("╠══════════════════════════════════════════════════════════════════╣")
    print("║  NEW: Data-driven filters from 33-trade analysis               ║")
    print("║  • XGBoost ML model (replaces Random Forest)                   ║")
    print("║  • RSI filter: no trading against extreme momentum             ║")
    print("║  • Max entry 63¢: better risk/reward ratio                     ║")
    print("║  • Requires book signal OR high ML confidence                  ║")
    print("║  • No late entries (after minute 8)                            ║")
    print("║  • Min EV raised to 8%                                        ║")
    print("╠══════════════════════════════════════════════════════════════════╣")
    print("║  MODES:                                                         ║")
    print("║  python3 bot_v65.py --trade    # Trade with picky filters     ║")
    print("║  python3 bot_v65.py --book     # Watch order book in real-time ║")
    print("║  python3 bot_v65.py --analyze  # Analyze trade history         ║")
    print("╚══════════════════════════════════════════════════════════════════╝")
    print()
    ...
    if len(sys.argv) < 2:
        print("Usage:")
        print("  python3 bot_v65.py --trade    # Trade with picky filters")
        print("  python3 bot_v65.py --book     # Watch order book live")
        print("  python3 bot_v65.py --analyze  # Analyze past trades")
        return

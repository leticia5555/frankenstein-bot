...
async def main():
    global position, candle_open, tokens, last_candle_time, recent_prices
    
    print("=" * 60)
    print("  BOT v8 - MOMENTUM + SURE THING MODE")
    print("=" * 60)
    ...
    print(f"Sure Thing: BTC ≥{SURE_THING_BTC_MOVE}% OR price ≥{SURE_THING_PRICE_TRIGGER*100:.0f}c")
    print("=" * 60)
            ...
            async with websockets.connect(
                ...
                async for msg in ws:
                    ...
                    if position:
                        ...
                        print(f"HOLD {position['side']} | {position['entry_price']*100:.0f}c→{curr*100:.0f}c ({price_diff*100:+.0f}c) | ~${est_pnl:+.2f} | BTC:{btc_change:+.2f}% {mom_str}")
                    ...
                    # LOOKING FOR ENTRY
                        ...
                        # Based on trader strategy: if price ≥85c, market has decided!
                        ...
                        # Method 2: Price is very high (≥85c) = market has decided!
                        # Only need momentum to confirm direction
                        elif m["up"] >= SURE_THING_PRICE_TRIGGER and m["up"] <= SURE_THING_MAX_PRICE and momentum == "UP":
                            sure_thing = True
                            sure_side = "UP"
                            sure_token = m["up_t"]
                            sure_price = m["up"]
                        
                        elif m["dn"] >= SURE_THING_PRICE_TRIGGER and m["dn"] <= SURE_THING_MAX_PRICE and momentum == "DOWN":
                            sure_thing = True
                            sure_side = "DN"
                            sure_token = m["dn_t"]
                            sure_price = m["dn"]
                        ...
                        if sure_thing and (1 - sure_price) >= SURE_THING_MIN_PROFIT:
                            ...
                            buy(sure_token, sure_price, sure_side)

...
# === GABAGOOL STRATEGY v38 (HEDGE + DOUBLE DOWN) ===
# Phase 1: Try to hedge (insurance)
# Phase 2: When winner is clear (70c+), buy MORE of winner!
# The winner profit covers hedge slippage loss

TOTAL_BUDGET = 8              # Total $ to spend per candle
FIRST_BUY = 2                 # First hedge buy
HEDGE_BUY = 2                 # Second hedge buy  
WINNER_BUY = 3                # Buy more when winner is clear
...
WINNER_THRESHOLD = 0.70       # Buy more when price hits 70c+
...
# Track if we already doubled down
doubled_down = False
...
def is_last_phase():
    return get_minutes_remaining() <= 5
...
def show_position_status():
    ...
    if up_shares > 0 and dn_shares > 0:
        ...
        if up_shares > dn_shares:
            print(f"       📈 EXTRA UP: {up_shares - dn_shares:.2f} shares (double down!)")
        elif dn_shares > up_shares:
            print(f"       📉 EXTRA DN: {dn_shares - up_shares:.2f} shares (double down!)")
    elif up_shares > 0:
...
def settle_positions(btc_final_change):
    ...
    global up_position, dn_position, doubled_down
    ...
    # Reset doubled_down for next candle
    doubled_down = False
    ...
    session_pnl += pnl
    candles_traded += 1
    print(f"\nSession: {candles_traded} trades ({hedged_trades} hedged, {doubledown_trades} doubled)")
...
async def main():
    ...
    print("=" * 60)
    print("  BOT v38 - HEDGE + DOUBLE DOWN")
    print("  1. Hedge both sides (insurance)")
    print("  2. When winner hits 70c+, buy MORE!")
    print("=" * 60)
    print(f"Budget: ${TOTAL_BUDGET}")
    print(f"Hedge buys: ${FIRST_BUY} + ${HEDGE_BUY}")
    print(f"Double down: ${WINNER_BUY} when winner ≥ {WINNER_THRESHOLD*100:.0f}c")
            ...
            async with websockets.connect(
                ...
                async for msg in ws:
                    ...
                    if is_hedged():
                        ...
                        print(f"👀 | UP:{up_price*100:.0f}c DN:{dn_price*100:.0f}c | BTC:{btc_change:+.2f}% ({momentum}) | {minutes_left:.1f}m | {reason}")
                    ...
                    # === PHASE 2: DOUBLE DOWN ON WINNER ===
                    # When hedged and one side hits 70c+, buy MORE of that side!
                    if is_hedged() and not doubled_down and remaining >= 1:
                        
                        if up_price >= WINNER_THRESHOLD:
                            # UP is winning! Buy more UP!
                        ...
                        elif dn_price >= WINNER_THRESHOLD:
                            # DN is winning! Buy more DN!
                            buy_amount = min(WINNER_BUY, remaining)

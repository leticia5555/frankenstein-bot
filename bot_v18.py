...
MAX_PRICE = 0.55         # Don't buy if price > 55c (too expensive)
...
edge_trades_this_candle = 0     # Count edge trades (max 3 per candle)
MAX_EDGE_TRADES = 3             # Limit edge trades per candle
SLUG = None
tokens = None
candle_open = None
...
def check_for_new_candle():
    global last_candle_time, candle_open, recent_prices, has_traded_this_candle, edge_trades_this_candle
    ...
    if new_time != last_candle_time:
        ...
        edge_trades_this_candle = 0     # Reset edge trade counter
        return True
    return False


def check_for_new_market():
    global position, recent_prices, position_is_sure_thing, has_traded_this_candle, edge_trades_this_candle
    ...
    if is_new:
        ...
        edge_trades_this_candle = 0     # Reset for new market
        return True
    return False


async def main():
    global position, candle_open, tokens, last_candle_time, recent_prices, position_is_sure_thing, has_traded_this_candle, edge_trades_this_candle
    
    print("=" * 60)
    print("  BOT v18 - MULTIPLE EDGE TRADES PER CANDLE")
    print("=" * 60)
    print(f"Min Edge: {MIN_EDGE*100:.0f}c | Max Price: {MAX_PRICE*100:.0f}c (dynamic)")
    print(f"Max Edge Trades: {MAX_EDGE_TRADES} per candle")
    print(f"TP: +{PROFIT_TARGET*100:.0f}c | SL: -{STOP_LOSS*100:.0f}c")
    print(f"Min Momentum: {MIN_MOMENTUM}% from candle open")
    print(f"Sure Thing: BTC ≥{SURE_THING_BTC_MOVE}% OR (price ≥80c + BTC ≥0.03%)")
    print(f"Arbitrage: DISABLED")
    print("=" * 60)
            ...
            async with websockets.connect(
                ...
                async for msg in ws:
                    ...
                    if position:
                        ...
                        # Show if this is a Sure Thing (will hold to expiry)
                        ...
                        # Exit conditions - BUT NOT FOR SURE THING POSITIONS!
                    ...
                    # LOOKING FOR ENTRY
                        ...
                        # Dynamic MAX_PRICE: bigger edge = allow higher entry price
                        # Base: 55c, but add extra room for edges above minimum
                        # e.g., 12c edge = 55c max, 20c edge = 63c max, 27c edge = 70c max
                        adjusted_max_price = MAX_PRICE + (best_edge - MIN_EDGE)
                        adjusted_max_price = min(adjusted_max_price, 0.75)  # Never pay more than 75c
                        
                        # Entry: edge good + price within adjusted max + momentum confirms
                        if best_edge >= MIN_EDGE and best_price <= adjusted_max_price:
                            if edge_trades_this_candle >= MAX_EDGE_TRADES:
                                print(f"    ⏸ SKIP: Already {edge_trades_this_candle} edge trades this candle (max {MAX_EDGE_TRADES})")
                            elif can_trade:
                                print(f"*** {best_side} SIGNAL: +{best_edge*100:.0f}c edge @ {best_price*100:.0f}c (max {adjusted_max_price*100:.0f}c) | MOMENTUM ✓ | Edge #{edge_trades_this_candle+1} ***")
                                if buy(best_token, best_price, best_side):
                                    edge_trades_this_candle += 1
                        ...
                        elif best_edge >= MIN_EDGE and best_price > adjusted_max_price:
                            print(f"    ⏸ SKIP: Price {best_price*100:.0f}c > max {adjusted_max_price*100:.0f}c")

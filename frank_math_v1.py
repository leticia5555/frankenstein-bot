                ...
                while not shutdown_event.is_set():
                    ...
                    if c and c.get("up_token"):
                        sub = json.dumps({"type":"subscribe","channel":"book",
                            "assets_ids":[c["up_token"],c.get("dn_token","")] })
                        await ws.send(sub)
                    ...
                    if isinstance(d, list):
                        for item in d:
                            ...
                            p = item.get("price") or (item.get("asks",[{}])[0] if item.get("asks") else {}).get("price")
                            if p:
                                try:
                                    pf = float(p)
                                    if aid==c.get("up_token"): pm_buffer.append((ts_ms,pf,None))
                                    elif aid==c.get("dn_token"): pm_buffer.append((ts_ms,None,pf))
                                except: pass
        except: pass
        if not shutdown_event.is_set():
            await asyncio.sleep(2)

async def rest_poller(shutdown_event):
    while not shutdown_event.is_set():
        try:
            c = candle_state.get("candle")
            if c and c.get("up_token"):

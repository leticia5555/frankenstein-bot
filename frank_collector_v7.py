...
async def signal_loop(shutdown_event):
    ...
    while not shutdown_event.is_set():
        ...
        # ── T=0: Capture BTC price immediately ──
        spot_start = get_btc_now()

        loop = asyncio.get_event_loop()

        # Fetch candle info + CLOB in parallel (klines/enrichments already prefetched)
        async def _get_clob(ci):
            if ci and ci.get("up_token"):
                return await loop.run_in_executor(None, get_clob_prices_rest,
                    ci["up_token"], ci.get("dn_token"))
            return None, None

        candle_info = await loop.run_in_executor(None, find_candle)
        candle_state["candle"] = candle_info

        # Now fetch CLOB (and klines/enrichments if not prefetched) in parallel
        clob_task = loop.run_in_executor(None, get_clob_prices_rest,
            candle_info["up_token"] if candle_info else None,
            candle_info.get("dn_token") if candle_info else None)
        if not klines_pre:
            klines_task = loop.run_in_executor(None, get_klines, 120)
            enrich_task = loop.run_in_executor(None, get_enrichments)
            klines, enrichments, clob_result = await asyncio.gather(klines_task, enrich_task, clob_task)
        else:
            klines = klines_pre
            enrichments = enrichments_pre
            clob_result = await clob_task

        up_ask_init = dn_ask_init = None
        if clob_result and clob_result[0] and clob_result[1]:
            up_ask_init, dn_ask_init = clob_result
